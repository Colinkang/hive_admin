...
import { ADMIN_HIVE_API_TOKEN, ADMIN_HIVE_USER_NAME,ADMIN_HIVE_USER_RIGHTS } from '../../common/localStorageKey';
			...
			if (this.account === '' || this.password === '') {
				...
				result.then(res => {
					...
					if (res.data.responseCode === '000000') {
						...
						this.text = '验证成功，欢迎登陆';
						//保存token到local
						LocalStore.setItem(ADMIN_HIVE_API_TOKEN, res.data.data.authToken);
						LocalStore.setItem(ADMIN_HIVE_USER_NAME, this.account);
						LocalStore.setItem(ADMIN_HIVE_USER_RIGHTS,res.data.data.rights);
						setTimeout(() => {
							this.$emit('login-success');
						}, 1000);
					} else {
