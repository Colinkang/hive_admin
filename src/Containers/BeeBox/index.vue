...
<div class="outer-box">
  <div class="hive-top">
    ...
    <div class="hive-top-input">
      <el-input size="mini" placeholder="请输入查询内容" v-model="search" clearable prefix-icon="el-icon-search" @keyup.enter.native="searchByKeyWord"></el-input>
    </div>
  </div>
  <div class="box">
    ...
    <div class="section-right">
      <div class="section-right-top">
            ...
            <div class="overview-chart">
              <div class="overview-chart-left">
                <div class="overview-chart-left-row">
                  <div class="section-title" style="text-indent:0;font-size:17px">
                    总览
                  </div>
        ...
        <div class="section-right-top-right">
          ...
          <baidu-map style="width:96%;height:93%;margin-left:2%;margin-top:2%;" :center="lng&&lat?{lng, lat}:'北京'" :zoom="zoom">
          ...
          <div class="form-row group-table">
            <table border="0" style="border:none">
              <tr style="border:none;background:#40577f;color:white">
                ...
                <th style="border:none;width:50%;color:white">名称</th>
...
export default {
	...
	methods: {
		...
		// 按照id来排序
		sortById() {
			this.array1 = sortBy('beeBoxNo', this.statusList, this.checkAllStatus, [], this.hiveList, true);
			this.beeBoxSortList = [];
			this.$nextTick(() => {
		...
		// 按照温度来排序
		sortByTemperature() {
			this.array1 = sortBy('temperature', this.statusList, this.checkAllStatus, [], this.hiveList, true);
			this.beeBoxSortList = [];
			this.$nextTick(() => {
				this.hiveList = this.beeBoxSortList.concat(this.array1);
				this.array1 = [];
		...
		// 按照湿度来排序
		sortByHumidity() {
			this.array1 = sortBy('humidity', this.statusList, this.checkAllStatus, [], this.hiveList, true);
			this.beeBoxSortList = [];
			this.$nextTick(() => {
				this.hiveList = this.beeBoxSortList.concat(this.array1);
				this.array1 = [];
		...
		// 按照重量来排序
		sortByGravity() {
			this.array1 = sortBy('gravity', this.statusList, this.checkAllStatus, [], this.hiveList, true);
			this.beeBoxSortList = [];
			this.$nextTick(() => {
				this.hiveList = this.beeBoxSortList.concat(this.array1);
				this.array1 = [];
		...
		// 按照压强来排序
		sortByAirPressure() {
			this.array1 = sortBy('airPressure', this.statusList, this.checkAllStatus, [], this.hiveList, true);
			this.beeBoxSortList = [];
			this.$nextTick(() => {
				this.hiveList = this.beeBoxSortList.concat(this.array1);
				this.array1 = [];
		...
		// 按照状态来排
		sortByStatus() {
			this.array1 = sortBy('status', this.statusList, this.checkAllStatus, [], this.hiveList, true);
			this.beeBoxSortList = [];
			this.$nextTick(() => {
				this.hiveList = this.beeBoxSortList.concat(this.array1);
				this.array1 = [];
		...
		sortByBeeBoxNum() {
			...
			this.groupSortList = [];
			this.$nextTick(() => {
				this.groupList = this.groupSortList.concat(this.array1);
				this.array1 = [];
			});
		},
		//获取蜂箱默认信息
		...
		createStartData(res) {
			...
			if (data.length > 0) {
				...
				for (let i = 0; i < this.hiveList.length; i++) {
					const position = {
						lng: this.hiveList[i].lng,
						lat: this.hiveList[i].lat,
					};
		...
		deleteGroupList() {
			...
			let result = post('/deleteGroups', {
				ids: this.deleteGroupIdArray,
			});
			result.then(res => {
				if (res.data.responseCode === '000000') {
					this.$message({
